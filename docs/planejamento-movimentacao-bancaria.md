...
- A classificação bancária dos 20 títulos foi concluída localmente por correspondência exata de data e valor com os OFX. Os quatro pagamentos pertencem ao Sicoob. Dos 16 recebimentos, dez pertencem ao Bradesco e seis ao Sicoob. Créditos agrupados foram preservados como conciliações 1:N: R$ 478,50 (dois títulos), R$ 1.650,00 (dois), R$ 1.877,60 (quatro), R$ 1.698,90 (três) e R$ 898,90 (três).
- Antes da gravação foi criado `storage/app/backups/isabelle_db_pre_ofx_homologacao_20260910.sql`, SHA-256 `8bebe3cb06817516f820025eb907dff3edb8972fc48e1ec8bf33dbb525818b30`. Os dois OFX foram importados nas contas correspondentes: 35 linhas no total. As 11 linhas que fecham os 20 títulos foram conciliadas; restaram 24 linhas sem título, compostas por tarifas, rendimentos e outros pagamentos/transferências. Elas permanecem pendentes para classificação contábil e conferência documental, sem inferência automática.
- O procedimento foi repetido e manteve exatamente 20 baixas, 20 movimentos, duas importações, 35 linhas e 11 conciliações, confirmando idempotência. Não existem mais títulos pagos desde o corte sem baixa bancária no espelho local.
- A suíte completa passou em SQLite isolado com **69 testes aprovados, 336 assertions e dois testes de concorrência ignorados por exigirem MySQL**. Esses dois cenários também passaram no MySQL isolado; a validação bancária integrada terminou com **39 testes aprovados e 230 assertions**. O teste antigo da raiz foi corrigido para validar o redirecionamento esperado de visitante para `/login`.
- Após conciliar os títulos, o saldo do sistema em 03/09/2026 ainda difere do OFX em R$ 6.228,24 no Bradesco e R$ 4.067,58 no Sicoob. Essas diferenças correspondem às 24 linhas pendentes e não indicam erro nas 11 conciliações concluídas. A homologação funcional das telas pode continuar agora; a homologação de fechamento diário e a publicação em produção dependem da identificação dessas operações e da confirmação dos responsáveis com permissão para estornar, desfazer conciliação e corrigir saldo inicial.
