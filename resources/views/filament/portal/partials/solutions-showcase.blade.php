@php
    ...
    $whatsappMessage = 'Olá! Acessei o Portal do Cliente e quero conhecer as soluções do Instituto Alves Neves.';
    $contactUrl = 'https://wa.me/5563981388139?text=' . rawurlencode($whatsappMessage);

    $solutions = [
        [
            'eyebrow' => 'Conformidade Legal',
            'title' => 'NR-1',
            'description' => 'Cuidado psicossocial com método, evidências e acompanhamento para sua empresa.',
        ...
        [
            'eyebrow' => 'Pessoas que transformam',
            'title' => 'Formação Humana',
            'description' => 'Desenvolvimento aplicado para lideranças, gestores e equipes mais saudáveis.',
        ...
        [
            ...
            'title' => 'Treinamentos',
            'description' => 'Experiências de aprendizagem desenhadas para os desafios reais da sua organização.',
...
<section class="portal-solutions {{ $compact ? 'portal-solutions--compact' : '' }}" aria-labelledby="portal-solutions-title">
    <div class="portal-solutions__hero">
        <div class="portal-solutions__glow portal-solutions__glow--one"></div>
        <div class="portal-solutions__glow portal-solutions__glow--two"></div>

        <div class="portal-solutions__hero-content">
            <span class="portal-solutions__overline">
                <x-filament::icon icon="heroicon-m-sparkles" />
                Soluções Instituto Alves Neves
            </span>
            <h2 id="portal-solutions-title">Sua empresa pode ir além do cumprimento de normas.</h2>
            <p>Transformamos conformidade, desenvolvimento humano e aprendizagem em ambientes de trabalho mais seguros, saudáveis e preparados para crescer.</p>

            <div class="portal-solutions__actions">
                <a class="portal-solutions__primary-action" href="{{ $contactUrl }}" target="_blank" rel="noopener noreferrer">
                    Quero conhecer as soluções
                    <x-filament::icon icon="heroicon-m-arrow-up-right" />
                </a>
                <a class="portal-solutions__secondary-action" href="{{ $compact ? route('filament.portal.pages.solucoes') : '#solucoes-disponiveis' }}">
                    Ver áreas de atuação
                </a>
            </div>
        </div>

        <div class="portal-solutions__hero-mark" aria-hidden="true">
            <img src="{{ asset('images/logo.png') }}" alt="">
            <small>Estratégia com cuidado humano</small>
        </div>
    </div>

    @unless ($compact)
        <div id="solucoes-disponiveis" class="portal-solutions__grid">
            @foreach ($solutions as $solution)
                <article class="portal-solutions__card portal-solutions__card--{{ $solution['accent'] }}">
                    <div class="portal-solutions__card-header">
                        <div class="portal-solutions__icon">
                            <x-filament::icon :icon="$solution['icon']" />
                        </div>
                        <div>
                    ...
                    <p class="portal-solutions__description">{{ $solution['description'] }}</p>

                    <ul>
                        @foreach ($solution['items'] as $item)
                            <li>
                                <span><x-filament::icon icon="heroicon-m-check" /></span>
                                {{ $item }}
                            </li>
                        @endforeach
                    </ul>

                    <a href="{{ $contactUrl }}" class="portal-solutions__card-action" target="_blank" rel="noopener noreferrer">
                        Conversar sobre {{ $solution['title'] }}
        ...
        <div class="portal-solutions__closing">
            <div>
                <span>Uma solução pensada para o seu momento</span>
                <h3>Vamos construir o próximo passo juntos?</h3>
                <p>Conte-nos o desafio da sua empresa e conheça uma proposta sob medida.</p>
            </div>
